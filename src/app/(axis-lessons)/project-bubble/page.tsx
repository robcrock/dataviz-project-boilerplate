...
export default function ProjectBubblePage() {
  // 1. Access data
  const data = [
    {
      "country": "Afghanistan",
  ...
  // 4. Draw scales
  const xAccessor = d => d.gdpPercap
  const xScale = scaleLinear()
    .domain(extent(data, xAccessor))
    .range([0, dims.boundedWidth])
    .nice()

  const yAccessor = d => d.lifeExp
  const yScale = scaleLinear()
    .domain(extent(data, yAccessor))
    .range([dims.boundedHeight, 0])
    .nice()
    
  const sizeAccessor = d => d.pop
  const sizeScale = scaleLinear()
    .domain(extent(data, sizeAccessor))
    .range([5, 40])
  ...
  console.log('cS', colorScale('Asia'))
  ...
  return <svg width={dims.width} height={dims.height}>
    ...
    >
      ...
      <g style={{transform: `translate(0px, ${dims.margin.top}px)`}}>
        <AxisLeft yScale={yScale} pixelsPerTick={50} title={'Life expectancy'} />
      </g>
      <g style={{transform: `translate(0px, ${dims.boundedHeight + dims.margin.top}px)`}}>
        <AxisBottom xScale={xScale} pixelsPerTick={50} title={'Gdp per Capita'} formatString="," />
      </g>
    </g>  
  </svg>
}
